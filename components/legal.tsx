  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-10 px-6 pb-8 pt-4 sm:flex-row sm:items-stretch sm:gap-10 sm:px-10 sm:pb-6">
        {/* The documents. Two of them, so they are set at the size of a
            thing you choose between rather than a line in a list. */}
        <nav
          aria-label="Documents"
          className="flex shrink-0 flex-col gap-4 sm:w-[min(16rem,19%)]"
        >
          {docs.map((d) => {
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  here ? "border-foreground" : "hoverable:hover:border-foreground",
                )}
              >
                <span
                  className={cn(
